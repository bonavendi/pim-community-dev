...
namespace Acme\Bundle\CsvConnectorBundle\Job;
...
class Job implements JobInterface
{
    ...
    final public function execute(JobExecution $jobExecution)
    {
        try {
            ...
            $this->createAndPostSlackMessage($jobExecution);
    ...
    private function createAndPostSlackMessage(JobExecution $jobExecution)
    {
        $status = $jobExecution->getStatus();
        $label = $jobExecution->getLabel();
        $formatBaseMessage = "IMPORT JOB: %s \nSTATUS: %s %s";
        $failures = $jobExecution->getAllFailureExceptions();

        if (isset($status) && $status->getValue() == BatchStatus::COMPLETED && empty($failures)) {
            $message = sprintf($formatBaseMessage, $label, ":sunny:", "");
        } else {
            $errorMessage = "";

            if (!empty($failures)) {
                $errorMessages = array_column($failures, 'message');
                $errorMessage = implode("\n:point_right: ", $errorMessages);
            }

            if (!empty($errorMessage)) {
                $message = sprintf($formatBaseMessage, $label, ":zap:", "\nERRORS:\n:point_right: " . $errorMessage);
            } else {
                $message = sprintf($formatBaseMessage, $label, ":zap:", "\nERRORS:\nUnknown error");
            }
        }
        $this->postOnSlack($message);
    }


    private function postOnSlack(string $msg)
    {
        $url = "https://hooks.slack.com/services/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";
